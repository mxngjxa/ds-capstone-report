...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{report_files/mediabag/figures/LSTM_cell.pdf}}
...
remarkable 7.7\% absolute improvement on the GLUE benchmark(Wang et al.,
2018). This success demonstrated that bidirectional pre-training could
capture linguistic phenomena that were previously difficult for
...
strategies for adapting pre-trained models to specific
tasks(\emph{Fine-Tuning Strategies for Task Adaptation}, 2023). Key
...
years old(Lin et al., 2021). Drug use prevalence is higher among certain
groups such as students in night vocational high schools. Schools in
Taiwan play a central role in preventing and integrating anti-drug
education training into the curriculum, the efforts of which can be
supported by our web filter({``Substance Use Among Adolescents in
Taiwan,''} 2021).
...
Analysis/PCA) as the inference engine(Blei et al., 2003; Jolliffe, 2002;
Van der Maaten \& Hinton, 2008). A range of topic counts (from 15 to
150) will be used in order to observe whether there is a difference in
separation quality, and more importantly, whether there are topics that
are difficult to linearly separate in order to take note of CWCM-V2.
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{figures/placeholder.png}}
...
When looking at the confusion matrices for our models, it seems that
XGBoost generally has a greater tendency to struggle with outlier
classification and lower precision performance than the other two
models.
...
When comparing by category, it seems that LinearSVC has a slight edge
over the other models both in terms of reduced overfitting and general
...
\caption{Comparing finalist candidates for Logistic Regression Model: v0
(initial baseline), v2/v3 (optimized for high recall), and v5 (optimized
for f1-macro performance); top: per class train-test F1-macro score
comparison, middle: per class test F1-macro score comparison and
recall-precision trade-off with support indication, bottom: confusion
matrix for top 4 model candidates.}
...
\caption{Comparing mean F1-Scores and QQ plots for both models against
LR baseline metric.}
